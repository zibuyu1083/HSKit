...
- (NSDictionary *)dzn_buttonAttributesForEmptyDataSet:(UIControlState)state
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(buttonAttributesForEmptyDataSet:forState:)]) {
        NSDictionary *attrs = [self.emptyDataSetSource buttonAttributesForEmptyDataSet:self forState:state];
        if (attrs) NSAssert([attrs isKindOfClass:[NSDictionary class]], @"You must return a valid NSDictionary object for -buttonAttributesForEmptyDataSet:forState:");
        return attrs;
    }
    return nil;
}
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            if (buttonImage) {
                ...
                NSDictionary *attrs = [self dzn_buttonAttributesForEmptyDataSet:UIControlStateNormal];

                if (attrs) {
                    if ([attrs[@"layer.cornerRadius"] isKindOfClass:NSNumber.class]) {
                        view.button.layer.cornerRadius = [attrs[@"layer.cornerRadius"] floatValue];
                    }
                    
                    if ([attrs[@"layer.masksToBounds"] isKindOfClass:NSNumber.class]) {
                        view.button.layer.masksToBounds = [attrs[@"layer.masksToBounds"] boolValue];
                    }
                    
                    if ([attrs[@"backgroundColor"] isKindOfClass:UIColor.class]) {
                        view.button.backgroundColor = attrs[@"backgroundColor"];
                    }
                }else{
                    view.button.layer.cornerRadius = 0;
                    view.button.layer.masksToBounds = NO;
