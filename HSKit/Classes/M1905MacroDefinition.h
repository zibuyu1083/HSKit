...
#define BUY_APPLEID     @"BUY_APPLEID"
...
// Tabbar高度
#define M1905_TAB_BAR_HEIGHT ([UIDevice currentDevice].isIPhoneX ?83.f :49.f)

// StatusBar高度
//状态栏高度
#define M1905_STATUS_BAR_HEIGHT  ([UIDevice currentDevice].isIPhoneX ?44.f :20.f)
//导航栏高度
#define M1905_NAVI_BAR_HEIGHT (self.navigationController.navigationBar.frame.size.height)
//状态栏 + 导航栏高度
#define M1905_TOP_BAR_HEIGHT (self.navigationController.navigationBar.frame.size.height + [[UIApplication sharedApplication] statusBarFrame].size.height)
//Tabbar高度
#define M1905_BOTTOM_BAR_HEIGHT (self.tabBarController.tabBar.frame.size.height)
//有导航栏和Tabbar的页面高度
#define M1905_PAGE_HEIGHT ([UIScreen mainScreen].bounds.size.height - M1905_TOP_BAR_HEIGHT - M1905_BOTTOM_BAR_HEIGHT)

//首页分割线高度

#define M1905_Line_Height  0.5
...
#define M1905_DEVICE_WIDTH MIN(M1905_SCREEN_WIDTH, M1905_SCREEN_HEIGHT)
#define M1905_DEVICE_HEIGHT MAX(M1905_SCREEN_WIDTH, M1905_SCREEN_HEIGHT)
...
static inline CGFloat CGFloatAutoFit (CGFloat value){
    CGFloat width = MIN(M1905_SCREEN_WIDTH, M1905_SCREEN_HEIGHT);
    return width / 375 * value;
}

static inline CGFloat CGFloatAutoFitSmall (CGFloat value){
    CGFloat width = MIN(M1905_SCREEN_WIDTH, M1905_SCREEN_HEIGHT);
    if (width/375>= 1) {
        return value;
    }
    width = MIN(M1905_SCREEN_WIDTH, M1905_SCREEN_HEIGHT);
    return width / 375 * value;
}

static inline CGFloat CGFloatMultiple16_9 (CGFloat width){
    return width / 16 * 9;
}
...
#define M1905_HeaderImage  [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"headerImage/%@.png",[UserTransaction sharedInstance].currentUser.usercode]]
...
#define UIColorFromRGB M1905_UIColorFromRGB
...
#define M1905_AppDelegate ((AppDelegate*)[UIApplication sharedApplication].delegate)
...
#define M1905_SDIMAGEOPTIONS SDWebImageRetryFailed|SDWebImageLowPriority

//审核模式
#define M1905_AuditModle ([[UserTransaction sharedInstance] auditModle])
//天翼视讯
#define M1905_TYModle ([[UserTransaction sharedInstance] TYModel])
//UUID
#define M1905_UUID [SNUUID UUID]

//----------------------全局消息定义----------------------------
#define DY_NOTIF_LOGIN_SUCCESS_UPDATEINFO @"loginSuccessUpdateInfo"
